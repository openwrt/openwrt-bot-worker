...
import { calculateHmac, parseGraphqlRequest, graphqlResponse, graphqlLabelsHandler, restListJson, restPages, graphqlCommitPageHandler, graphqlCommitFromRest, graphqlCommentFromRest } from './github-mocks.js';
...
describe('Cloudflare Worker Webhook & Error Handling', { concurrency: 1 }, () => {
  ...
  test('registers the run with waitUntil so a client hanging up cannot cut it short', async () => {
    const registered = [];
    const response = await worker.fetch(new Request('http://localhost/webhook', {
      method: 'POST',
      body: '{}',
      headers: { 'x-hub-signature-256': 'sha256=' + 'a'.repeat(64), 'x-github-event': 'pull_request' }
    }), { WEBHOOK_SECRET: 'secret' }, { waitUntil: (promise) => { registered.push(promise); } });
    assert.strictEqual(response.status, 403);
...
`;
  ...
  test('skips a repository whose listing came back with errors, even alongside data', async () => {
    const apiCalls = [];
    const daysAgo30 = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
    fetchMock = async (url, options) => {
      apiCalls.push({ url, method: options?.method || 'GET' });
      if (url.includes('/access_tokens')) return new Response(JSON.stringify({ token: 'inst-token' }), { status: 200 });
      if (url.includes('/app/installations')) return new Response(JSON.stringify([{ id: 101, account: { login: 'testorg' } }]), { status: 200 });
      if (url.includes('/installation/repositories')) return new Response(JSON.stringify({ repositories: [{ full_name: 'testorg/repo1' }] }), { status: 200 });
      {
        // GitHub resolved the pull request but not its timeline: HTTP 200,
        // the data it has, and an errors entry for the rest.
        const sr = staleGraphqlHandler(url, options, {
          config: { enable_stale_bot: true },
          prs: [{ number: 321, updatedAt: daysAgo30, labels: ['stale', 'not following guidelines'], timeline: [] }],
          errors: [{ message: 'Something went wrong while executing your query.', path: ['repository', 'pullRequests', 'nodes', 0, 'timelineItems'] }]
        });
        if (sr) return sr;
      }
      return new Response(JSON.stringify({}), { status: 200 });
    ...
    try {
      await handleScheduled({ APP_ID: '12345', PRIVATE_KEY: privateKeyPEM });
      assert.ok(!apiCalls.some(c => c.url.includes('/pulls/321') && c.method === 'PATCH'), 'must not close on a history it never saw');
      assert.ok(!apiCalls.some(c => c.url.includes('/issues/321/labels/stale') && c.method === 'DELETE'));
    } finally {
      fetchMock = null;
    }
  });
...
`;
  ...
  async function runWithLabelRemovalAnswer(answer, { action = 'opened', labels = ['not following guidelines'] } = {}) {
    const payload = JSON.stringify({
      action,
      pull_request: {
        number: 123,
  ...
  test('label removals go out one after another, not as a burst', async () => {
    let inFlight = 0;
    let burst = false;
    const { status, removals } = await runWithLabelRemovalAnswer(async () => {
      inFlight++;
      if (inFlight > 1) burst = true;
      await new Promise(resolve => setTimeout(resolve, 1));
      inFlight--;
      return new Response('[]', { status: 200 });
    }, { action: 'synchronize', labels: ['stale', 'not following guidelines'] });
    assert.strictEqual(status, 200);
    assert.deepStrictEqual(removals.sort(), ['not following guidelines', 'stale']);
    assert.strictEqual(burst, false);
  });
...
`).join('');
  ...
  // A pull request of more than 15 commits is judged from its whole patch.
  // Runs one webhook on such a pull request, with every file lookup failing,
  // and returns the check-runs it posted.
  async function checkRunsForWholePatch(prPatchText) {
    const payload = JSON.stringify({
      action: 'opened',
      pull_request: {
    ...
    fetchMock = async (url, options) => {
      ...
      { const lr = graphqlLabelsHandler(url, options, []); if (lr) return lr; }
      if (url.includes('/pulls/123/commits')) return new Response(JSON.stringify(commits), { status: 200 });
      if (url === 'https://api.github.com/repos/test/repo/pulls/123') return new Response(prPatchText, { status: 200 });
      if (url.includes('/check-runs') && method === 'POST') { checkRunsPosted.push(JSON.parse(options.body)); return new Response('{}', { status: 201 }); }
      if (url.includes('/issues/123/comments')) return new Response(JSON.stringify([]), { status: 200 });
      return new Response(JSON.stringify({}), { status: 200 });
    };
  ...
  test('reports neutral for an unreadable patch file when judging the whole patch too', async () => {
    const runs = await checkRunsForWholePatch(mailEnvelope('a'.repeat(40)) + modifyPatchFile);
  ...
  test('does not call a patch file that a later commit deletes unreadable', async () => {
    ...
    const runs = await checkRunsForWholePatch(mailEnvelope('a'.repeat(40)) + modifyPatchFile + mailEnvelope('b'.repeat(40)) + deletePatchFile);
    const patches = runs.find(r => r.name === 'FormalityCheck / Code Patches');
