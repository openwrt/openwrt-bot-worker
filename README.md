...
*   **Subject String Hygiene:** Enforces `<package name or prefix>: ` prefix headers (including source tree paths such as `tools/cmake: ` or `toolchain/musl: `), checks lowercase starting strings post-prefix, rejects trailing periods, and requires a blank line between the subject and the description body.
...
*   **Mandatory Metadata (`check_openwrt_meta`):** Enforces the inclusion of `PKG_MAINTAINER`, `PKG_LICENSE`, and `PKG_LICENSE_FILES` variables whenever a new package is introduced (fully configurable list). New LuCI packages (built on `luci.mk`) are only asked for `PKG_LICENSE` — their maintainer field is generated by `luci.mk` and they carry SPDX headers instead of license files.
...
*   **Conffiles Tracker:** Mandates the definition of the `Package/.../conffiles` tracking macro whenever configuration is installed — an `INSTALL_CONF` line or a destination under `$(1)/etc/config`. Other payload under `/etc` (capability files, `profile.d` snippets, …) is not configuration and does not trigger the demand. Entries themselves are checked for indentation and for being absolute paths, and an individual file must not carry a trailing slash — `scripts/ipkg-build` runs `find` over each entry and `package-pack.mk` keeps what `[ -f ]` accepts, so `/etc/config/foo/` matches neither and the file is silently left untracked. A directory reads the same to both with or without the slash, so neither spelling is asked for. Lines holding a make expansion (`$(CONF_DIR)/my.cnf`, `$(if $(CONFIG_…),…)`, a whole block pulled in as `$(Package/busybox/conffiles/crond)`) are paths only after the build resolves them and are left to it.
...
*   **Package Root Resolution:** Resolves the package a changed file belongs to by walking up past payload directories (`patches/`, `files/`, `src/`, and LuCI's `root/`, `htdocs/`, `luasrc/`, `ucode/`, `po/`). A directory sitting directly under a category is the package itself, not payload: `package/utils/ucode` is the ucode interpreter, and treating it as a LuCI payload directory used to resolve it to no package at all, so every pull request touching it went unaudited without saying so.
*   **PKG_RELEASE Validation:** Enforces correct release values on package changes: new packages must initialize `PKG_RELEASE` to `1`, version updates must reset `PKG_RELEASE` to `1`, and modifications to package files must be accompanied by a version/release change (customizable level: warning/error/disabled). Packages modified exclusively by revert commits are exempt from the reset/initialize rules, because a revert restores the version and release of an already released state. They must still carry a release bump if the revert changes package content without touching the version or release, otherwise users would never receive it. Packages built on a shared helper that never uses `PKG_RELEASE` (`u-boot.mk`, `trusted-firmware-a.mk`, `luci.mk` — LuCI versions come from git history) are exempt from all release requirements, and so is any package with no `PKG_RELEASE` and no version anyone could move: `package/kernel/linux` is the kernel itself and `package/kernel/bpf-headers` derives its `PKG_VERSION` from the kernel's, so asking either for a bump is advice nobody can follow. A package that still defines a literal `PKG_VERSION`, `PKG_SOURCE_VERSION` or `PKG_SOURCE_DATE` has something to bump and is asked for it as before.
