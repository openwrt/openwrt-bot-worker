...
*   **Conffiles Tracker:** Mandates the definition of the `Package/.../conffiles` tracking macro whenever configuration is installed — an `INSTALL_CONF` line or a destination under `$(1)/etc/config`. Other payload under `/etc` (capability files, `profile.d` snippets, …) is not configuration and does not trigger the demand. Each entry must be one unindented absolute path per line, or a make expansion that produces one; a file must not end with a slash, while a directory may be written with or without it.
