...
*   **HTTPS Source URLs (`check_source_url_https`):** Issues a non-blocking warning when a new `PKG_SOURCE_URL` uses plain `http://`, `git://` or `ftp://` — OpenWrt package policy ranks HTTPS above the plain transports. The content stays pinned either way — by `PKG_HASH`/`PKG_MIRROR_HASH` for archives, by `PKG_SOURCE_VERSION` for git checkouts; this is about transport hygiene, and the native `git://` protocol is no longer served by major code hosts at all (default true).
